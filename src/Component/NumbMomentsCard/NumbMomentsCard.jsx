...
export default function NumbMomentsCard ({username, days, session, isIndividual, emotion1, emotion2, sns1, sns2, averagewatchingtime}){
    return (
        ...
        <div className = 'numb-moments'>
            ...
            <div>
                <div><span className = 'username'>{username}</span>님은<span>{days}</span><span>{session}</span><span>{isIndividual}</span> 있을 때,</div>
                <div><span>{emotion1}</span> <span>{emotion2}</span> 감정을 느껴</div>
